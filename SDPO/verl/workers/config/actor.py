...
class SelfDistillationConfig(BaseConfig):
    ...
    def __post_init__(self):
        ...
        if self.alpha in {0.25, 0.75} and (
            not math.isfinite(self.rho)
            or self.rho <= 0.0
            or math.isclose(self.rho, 1.0, rel_tol=0.0, abs_tol=1e-12)
        ):
            raise ValueError(
                "self_distillation.rho must be finite, positive, and different from 1 "
                f"for Renyi objectives, got {self.rho}"
            )
        if self.renyi_regularization and not 0.0 <= self.renyi_regularization_level <= 1.0:
            raise ValueError(
                "self_distillation.renyi_regularization_level must be in [0,1], "
                f"got {self.renyi_regularization_level}"
            )
        ...
        if self.renyi_ref_sync_steps > 0 and not self.renyi_regularization:
            raise ValueError(
                "self_distillation.renyi_ref_sync_steps > 0 requires renyi_regularization=True"
            )
