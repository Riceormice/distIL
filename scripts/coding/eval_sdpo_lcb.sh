#!/bin/bash
# scripts/coding/eval_sdpo.sh
# Evaluate an SDPO checkpoint on LCBv6.
#
# Usage:
#   CHECKPOINT=SDPO-coding-lcbv6-Qwen-Qwen3-8B \
#   CHECKPOINT_STEP=80 \
#   REPO_DIR=/path/to/distil/SDPO \
#   bash scripts/coding/eval_sdpo.sh
...
# EVAL CONFIGURATION
# =============================================================================
CHECKPOINT="${CHECKPOINT:-SDPO-coding-lcbv6-Qwen-Qwen3-8B}"
...
EXP="${CHECKPOINT}"
LOG="${REPO_DIR}/../logs/eval_sdpo_lcb_${TIMESTAMP}.log"
SBATCH_SCRIPT="${REPO_DIR}/../logs/.sbatch_eval_sdpo_lcb_${TIMESTAMP}.sh"

mkdir -p "${REPO_DIR}/../logs"

cat > "${SBATCH_SCRIPT}" << SBEOF
#!/bin/bash
#SBATCH --job-name=eval-sdpo-lcb
...
bash ${REPO_DIR}/training/verl_training.sh '${EXP}' sdpo 'datasets/lcb_v6' \
  vars.dir=${REPO_DIR} \
