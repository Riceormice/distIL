...
class OPSDTrainer(SFTTrainer):
    ...
    def distil_loss(
        ...
        """
        DistIL: Full-RL Distillation Loss.

        Combines:
          Term 1: Cross-entropy CE(teacher || student) — direct imitation signal.
          Term 2: REINFORCE policy gradient with MiniLLM-style length-normalized
                  returns, where rewards are per-token cross-entropies.

        Args:
            student_logits: [B, T, V] raw student logits
            teacher_logits: [B, T, V] raw teacher logits (no grad)
            labels:         [B, T] with -100 for positions to ignore
            temperature:    softmax temperature
            top_k:          if set, restrict to top-k teacher tokens
            token_clip:     if set, clip per-token CE to this max
        ...
        loss_mask = (labels != -100).float()  # [B, T]
        ...
        # Term 1: CE(teacher || student)
        ce_per_token = -(teacher_probs * student_log_probs).sum(-1)  # [B, T]
        ce_per_token = ce_per_token.clamp(min=0.0)
        if token_clip is not None:
            ce_per_token = ce_per_token.clamp(max=token_clip)

        # Term 2: REINFORCE with future CE rewards (MiniLLM length normalization)
        with torch.no_grad():
            token_rewards = ce_per_token.detach()
            shifted = torch.cat(
            ...
            returns   = returns / remaining

        pg_per_token   = ce_per_token * returns
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.use_thinking_machines_loss:
            ...
            if self.loss_mode == "distil":
                loss, _distil_metrics = self.distil_loss(
    ...
    def log(self, logs: dict[str, float], start_time: float | None = None) -> None:
        ...
        if mode == "train":
            ...
            if hasattr(self, "_distil_metrics_accum") and self._distil_metrics_accum:
                logs.update(self._distil_metrics_accum)
                self._distil_metrics_accum = {}
