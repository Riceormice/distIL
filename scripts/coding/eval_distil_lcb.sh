#!/bin/bash
# scripts/coding/eval_distil.sh
# Evaluate a DistIL checkpoint on LCBv6.
#
# Usage:
...
# EVAL CONFIGURATION
# =============================================================================
# Name of the experiment checkpoint directory under ckpt_dir
CHECKPOINT="${CHECKPOINT:-DistIL-coding-lcbv6-Qwen-Qwen3-8B}"
CHECKPOINT_STEP="${CHECKPOINT_STEP:-80}"
MODEL_PATH="${MODEL_PATH:-Qwen/Qwen3-8B}"

# Eval uses temperature=0.2 as reported in paper (Table, Figure 2)
VAL_N=16
VAL_TEMPERATURE=0.2
VAL_TOP_P=0.95
VAL_TOP_K=20
...
EXP="${CHECKPOINT}"
LOG="${REPO_DIR}/../logs/eval_distil_lcb_${TIMESTAMP}.log"
SBATCH_SCRIPT="${REPO_DIR}/../logs/.sbatch_eval_distil_lcb_${TIMESTAMP}.sh"

mkdir -p "${REPO_DIR}/../logs"

cat > "${SBATCH_SCRIPT}" << SBEOF
#!/bin/bash
#SBATCH --job-name=eval-distil-lcb
...
bash ${REPO_DIR}/training/verl_training.sh '${EXP}' distil 'datasets/lcb_v6' \
  vars.dir=${REPO_DIR} \
